...
def convert(fname):
    ...
    with open(fname, 'rb') as file:  # should use uint instead of int...
        ...
        started_time = int.from_bytes(file.read(8), 'little') # unix time stamp (s)
        ...
        for sensor in range(sensor_count):
            ...
            calib_0[sensor] = struct.unpack('<f', file.read(4)) # could unpack em all in one shot
        ...
        clock = np.frombuffer(raw_clock, np.uint64)  # number of ticks since beginning of measure (to be confirmed)
